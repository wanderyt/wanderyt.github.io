...
`==`判断会自动转换数据类型，`===`比较时NaN不等于自身，而且`+0`和`-0`相等。
...
__操作中引入继承的属性会让问题复杂化，大多数时候，我们只关心对象自身的属性。所以，尽量不要用for...in循环，而用Object.keys()代替。__
...
返回不含继承的所有可枚举属性（不含Symbol属性）。

### Object.getOwnPropertyNames()

返回数组，包含自身所有属性（不含Symbol属性，包含不可枚举属性）。
...
返回数组，包含所有属性，不管属性名是Symbol或者字符串，也不管是否可枚举。

### Reflect.enumerate()

返回一个`Iterator`对象，遍历对象自身的和继承的所有可枚举属性（不含Symbol属性），与`for...in`循环相同。
